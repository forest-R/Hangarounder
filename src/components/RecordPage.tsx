...
function getDatesInRange(record: CampingRecord): string[] {
  if (!record.endDate) return [record.id];
...
export default function RecordPage({
  ...
  const memoDates = endDate
    ? eachDayOfInterval({ start: parseISO(dateId), end: parseISO(endDate) }).map(d => format(d, "yyyy-MM-dd"))
    : [dateId];
  ...
  const deleteRecord = async () => {
    ...
    setTitle(""); setLocation(""); setEndDate(""); setWeather(""); setMemos({}); setCheckedItems([]);
  };
      ...
      <div className="px-5 flex flex-col gap-3">
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100">
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-500">{format(parseISO(dateId), "M월 d일 (eee)", { locale: ko })}</span>
            <span className="text-gray-300">–</span>
            <input type="date" value={endDate} min={dateId}
              ...
              placeholder="종료일 (선택)" />
            {endDate && <button onClick={() => setEndDate("")} className="text-gray-300 text-xs">✕</button>}
          </div>
        </div>
            ...
            {WEATHER_OPTIONS.map(w => (
              <button key={w} onClick={() => setWeather(weather === w ? "" : w)}
                className={`px-3 py-1 rounded-full text-sm border transition-colors ${
                  weather === w ? "bg-forest-600 text-white border-forest-600" : "border-gray-200 text-gray-500"
                }`}>{w}</button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100">
          ...
          {memoDates.length > 1 && (
            <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
              {memoDates.map(d => (
                <button key={d} onClick={() => setActiveMemoDt(d)}
                  className={`flex-shrink-0 px-3 py-1 rounded-full text-xs border transition-colors ${
                    activeMemoDt === d ? "bg-forest-600 text-white border-forest-600" : "border-gray-200 text-gray-500"
                  }`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <button onClick={() => setGearOpen(o => !o)}
            ...
            <span className="text-gray-300 text-sm">{gearOpen ? "▲" : "▼"}</span>
          </button>
          {gearOpen && (
                        ...
                        return (
                          <button key={item.id} onClick={() => toggleItem(item.name)}
                            className={`px-3 py-1 rounded-full text-xs border transition-colors ${
                              checked ? "bg-forest-100 text-forest-800 border-forest-200" : "border-gray-200 text-gray-400"
                            }`}>
                            {checked ? "✓ " : ""}{item.name}
                          </button>
                        );
        ...
        <div className="flex gap-2 pt-1">
          ...
          {existing && (
            <button onClick={deleteRecord}
              className="px-4 bg-white border border-gray-200 text-gray-400 rounded-2xl py-3 text-sm active:bg-gray-50">
              삭제
            </button>
          )}
        </div>
