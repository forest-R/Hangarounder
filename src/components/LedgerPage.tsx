...
export default function LedgerPage({ entries, records }: { entries: LedgerEntry[]; records: CampingRecord[] }) {
  const [month, setMonth] = useState(new Date());
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ date: format(new Date(), "yyyy-MM-dd"), category: "식비" as LedgerEntry["category"], description: "", amount: "" });
  ...
  return (
    <div className="flex flex-col pb-6">
      ...
      <div className="flex items-center justify-between px-5 pt-6 pb-2">
        <div className="flex items-center gap-2">
          <button onClick={() => setMonth(m => { const d = new Date(m); d.setMonth(d.getMonth()-1); return d; })}
            className="w-7 h-7 flex items-center justify-center text-forest-600 rounded-full active:bg-forest-100">‹</button>
          <span className="text-xl font-medium text-forest-800">
            ...
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => setForm(f => ({ ...f, category: c }))}
                className={`px-3 py-1 rounded-full text-xs border transition-colors ${
                  form.category === c ? "bg-forest-600 text-white border-forest-600" : "border-gray-200 text-gray-500"
                }`}>{c}</button>
            ))}
          </div>
          <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
            className="text-sm text-gray-600 border border-gray-100 rounded-xl px-3 py-2 outline-none" />
          <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
            placeholder="항목 이름"
            className="text-sm text-gray-700 border border-gray-100 rounded-xl px-3 py-2 outline-none placeholder-gray-300" />
          <div className="flex gap-2 items-center">
            <input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))}
              placeholder="금액"
              className="flex-1 text-sm text-gray-700 border border-gray-100 rounded-xl px-3 py-2 outline-none placeholder-gray-300" />
      ...
      <div className="px-5 flex flex-col gap-2">
        {monthEntries.map(e => (
          <div key={e.id} className="flex items-center gap-3 bg-white rounded-2xl px-4 py-3 border border-gray-100">
            <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${CAT_COLORS[e.category]}`}>{e.category}</span>
            ...
            <span className="text-sm font-medium text-gray-700 flex-shrink-0">{e.amount.toLocaleString()}</span>
            <button onClick={() => del(e.id)} className="text-gray-200 text-xs active:text-red-400 ml-1">✕</button>
          </div>
        ))}
